    ...
    <div class="message-detail">
      ...
      <div class="message-header">
        <div class="breadcrumb">
          <Link :href="isInbox ? route('eleve.messages.inbox') : route('eleve.messages.sent')" class="back-link">
...
// Envoyer la réponse
const sendReply = () => {
  replyForm.post(route('eleve.messages.store'), {
    preserveScroll: true,
    onSuccess: () => {
...
const deleteMessage = () => {
  if (confirm('Voulez-vous vraiment supprimer ce message ? Cette action est irréversible.')) {
    router.delete(route('eleve.messages.destroy', props.message.id), {
      onSuccess: () => {
        // Rediriger vers la boîte de réception ou les messages envoyés
        if (isInbox.value) {
          router.visit(route('eleve.messages.inbox'))
        } else {
          router.visit(route('eleve.messages.sent'))
        }
      }
    })
  }
...
const viewMessage = (messageId) => {
  if (messageId !== props.message.id) {
    router.visit(route('eleve.messages.show', messageId))
  }
}
</script>
